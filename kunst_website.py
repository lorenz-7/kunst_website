...
if selection_screen == "Home":
    ...
    st.title("Reflexion")
    st.markdown('##### Meine Werkstattarbeit hatte das Thema "Wie kann "Kunst" in Interaktion mit der Natur helfen?" und durch diese Arbeit konnte ich viele Erfahrungen sammeln. Aufgrund meiner Vorerfahrungen und Interessen entschied ich mich, eine Website zu erstellen, die meine Ausarbeitung darstellt. Zunächst wollte ich ein Video erstellen, aber aufgrund der Komplexität und des geringen Nutzens habe ich mich für Kompromiss entschieden, Videos in meine Website zu integrieren. Nach zwei Wochen beschloss ich, eine Blog-Website zu gestalten, und nutzte dazu das Framework Streamlit, das mit der Programmiersprache Python funktioniert.')
